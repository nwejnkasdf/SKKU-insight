...
| 2 | **A7 leaf-lifecycle + traversal** | LifecycleEvaluator + D 하이브리드 + TraversalEngine(extend/retract/split/archive/**merge**) + leaf 재배치 LLM + 3단계 강등 + propagation 토글 | ⚠️ **부분 완료** (2026-05-17) — 7-commit PR-stack + Codex 3 라운드 23 fix. **단 trace operation 5종 중 `retract` 와 leaf-level `merge` 만 production caller 존재**. `extend` / `split` / `archive` 는 함수 정의 + 정적 단위 테스트만 있고 caller 누락 — 2026-05-20 30일 시뮬레이션 중 발견 ([decision-backlog P1-12](docs/decision-backlog.md)). before-fix 스냅샷: `A9-before-fix-extend` 브랜치. |
