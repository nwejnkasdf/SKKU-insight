...
| 2 | **A8 recommendation** | core/adjacent/discovery + fallback + Cold-start + first trace 생성 + emerging quota | ✅ **본문 완료** (2026-05-17, R2/R3 별도 세션) — 4-commit local PR-stack + R1 self-review fix |
...
마지막 갱신: 2026-05-17 (**Phase 2 A8 recommendation engine 본문 완료** — core/adjacent/discovery 3 슬롯 (5/3/2) + Cold-start LLM orchestrator + reason_short LLM batch + Document 섹션형 LLM 요약 + single-flight cache + 첫 trace 생성 hook (A7 결정 #6 plan TBD 완성)). 4 local commit-stack: c82ce83 PR-1 alembic 0006 + 3 ORM (Recommendation / RecommendationSlot / DocumentSummaryCache + daily UNIQUE + 3 CHECK) + 6476037 PR-2 본문 (`app/recommendation/` 11 파일 + `worker/jobs/cold_start.py` 본문 + `interest/service.py` trace creation hook + Settings 9 신규 + `recommendation.toml` + env-vars drift, ~3514 line) + ce35d8e PR-3 tests (9 파일 + conftest, 1852 line) + 15883d1 PR-4 R1 self-review fix (TopicChip dedup). **결정 매트릭스 7건** (decisions.md §13, decision-backlog C-40): PR-stack 7-commit 패턴 / Summary 캐시 = 신규 테이블 DocumentSummaryCache (사용자 결정 #2) / 첫 trace 생성 hook = cold_start orchestrator + ingest_event_atomic click hook 협업 (#3, A7 #6 plan TBD 완성) / PUT /onboarding/interests FR-55 = A8 범위 외 stub 유지 (#4, A9/A10 분담) / sentinel `cold_start_pseudo` + content_type='pseudo_cold_start' 활성화 / emerging quota = core 5 중 1 부재 시 active 회수 / score 컬럼 = Recommendation DB 영속 (admin 노출), 일반 사용자 응답 schema 부재 (NFR-04). **§11 anti-pattern 5건 사전 방어** (R1 fix 최소화 핵심): #1 cache-before-commit / #2 daily UNIQUE race / #3 emerging quota race / #4 NFR-04 score 노출 / #5 lock token race. **R1 self-review fix 1건**: TopicChip dedup by (topic_id, type). **R2/R3 후속 (별도 세션)**: R2 Codex 재감사 + R3 통합 시연 (실 OpenAI GPT-5.5 cold-start 1회 — signup→consent→onboarding 3 cluster→8s SLA→dashboard 10 카드 5/3/2 + NFR-04 score 미노출 + cache hit + click→trace 자동 생성). **다음 작업**: A9 electron-client (UI-01~05 + safeStorage + 한국어 i18n + codegen api.ts) — 또는 A8 R2 Codex 재감사 별도 세션 우선.

이전: 2026-05-17 (**Phase 2 A7 leaf-lifecycle + traversal 완료** — trace operation 4 → 5 (merge 신규) + D 하이브리드 + Strict 검증 + INTEREST_PROPAGATION_ENABLED=true 토글). 5 commit-stack: PR-1 contracts SOR (JobType.TRACE_MERGE + ErrorCode 5 + RedisKey 3 + Settings 33) + PR-2 alembic 0005 (UserCSOTraversal.merged_into_trace_id + ck_collection_job_type 갱신, P2-21 해소) + PR-3 본문 (app/traversal 5 + app/leaf_lifecycle 6 + worker/jobs 3 신규 + scheduler 3 cron + 47 unit test, ~3940 line) + PR-4 Codex R1 fix 10건 (Critical 3 + Suggested 6 + Nit 1) + PR-5 Codex R2 재감사 fix 6건 (Critical 3 + Suggested 2 + R1 deferred Suggested 5 처리). **결정 매트릭스 23건** (decisions.md §12, decision-backlog C-39): PR-stack 6-commit / Codex 3 라운드 (R1 + R2 + R3 통합 시연) / 실 GPT-5.5 시연 / propagation 토글 default true / 하이브리드 강등 (1단계 ingest 즉시 / 2-3단계 daily cron) / LLM Cap 폐지 (시연 단계) / alembic 0005 / Strict 검증 (confidence ≥0.6 + supporting ≥3 + anchor + label dedup) / trace_anchor retry cap=1 / merged leaf 추천 제외 / **trace merge operation 신규 도입** / input D union (A4 collection ∪ UserEvent click/save 24h) / split T 단축 + T'=분기점+B / merge winner=max activity + tie 시 trace_id 작은 쪽 / daily 18 UTC trace merge LLM cron. **Codex 16건 fix**: A6 anti-pattern 회피 (C-01 read-then-write / C-02 cache-before-commit / C-03 batch race / S-03 alpha floor / S-04 lifespan race) + 신규 (split child_A 누락 / lock key 통일 / path 길이 1 archive count / Lua atomic CAS / composite PK 충돌 / Strict anchor 1-hop / daily archive_threshold = stale+archive_after / status filter + RETURNING / recommendation_cache invalidate 4 worker / docs §12 SOR / scheduler docstring / mark_stale_if_idle hook). **다음 작업**: A8 recommendation (current/adjacent/discovery + Cold-start + first trace 생성 + emerging quota).
