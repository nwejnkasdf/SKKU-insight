...
| **A8-v2 — UserProfile + Discovery Fusion + Reincarnation Pivot** | ✅ 완료 ([PR #25](https://github.com/nwejnkasdf/SKKU-insight/pull/25) merge `63f2cdde`, 2026-05-19, C-42) — discovery slot 2 본질을 "trust=high trend" → "Fusion 1 (archive × current cross-product) + Reincarnation 1 (`score_tail >= 0.6` archived trace 부활)" 로 pivot. SRS FR-41 "잠재적으로 관심 있을 수 있는" 의도 회복. core 5 + adjacent 3 안정성 base 유지. 학술 정합: PersonaX (ACL'25) + LettinGo (KDD'25) + Serendipity 3-dim framework + 본 라운드 고유 archive×current cross-product 융합 angle. **본문**: alembic 0007 (UserProfile 10 컬럼 + ck_collection_job_type 7-value) + `app/profile/` 5 파일 (~900줄, Pydantic strict USER_PROFILE_JSON_SCHEMA) + `worker/jobs/user_profile.py` daily 19 UTC cron + scheduler 7th entry + recommendation engine pivot (`_build_discovery_pools` per-source fallback + `_build_fusion_subslot` + `_build_reincarnation_subslot` + `_resolve_seed_id`) + traversal queries 3 신규. **Codex R1 audit (codex:rescue GPT-5.5)**: Critical 2 + Suggested 7 + Nit 2 = 11 issue → R1 fix 7건 (lock TTL 360s / pool 분리 + slot 별 1개씩 / cache invalidate 분리 / trace_path 제외 / doc rows 기반 fallback / alembic downgrade NotImplementedError / prompt 자연어) + P2-26~29 backlog. **테스트**: tests/profile/ 4 파일 — **57 passed**. **WSL docker compose 통합 시연**: alembic 0001→0007 + lifespan 부트 (`cso_graph 14707 nodes / 12 clusters`) + scheduler `JOB_REGISTRATIONS=7` + 모든 신규 모듈 surface import + signup/login/consent/clusters HTTP OK. decisions.md §15 + 사용자 결정 11 + 자체 결정 10 |
| Phase 3 — A9 electron-client / A10 admin-console | 🟡 진행 중 (main 다수 보강 머지 — A9 ~30+ 커밋, A10 ~15+ 커밋. 시연 통과 검수 보류) |
...
**다음 액션**: A9 electron-client 본문 구현 ([`prompts/08-A9-electron-client.md`](prompts/08-A9-electron-client.md)) — UI-01~05 + Electron `safeStorage` (OS 키체인) + 한국어 i18n + `client/src/generated/api.ts` codegen 결과 import (raw fetch 금지). A8 + A8-v2 완료로 백엔드 4 endpoint (`/recommendations/dashboard{,refresh}`, `/documents/{id}{,/summary}`) 안정 — A9 가 직접 호출 + 카드 클릭 시 `POST /events {event_type=click}` 가 trace 자동 생성 (A8 hook). A8-v2 의 discovery Fusion + Reincarnation 카드도 동일 dashboard endpoint 로 자연 노출 (전용 reason_short 한 줄 — "두 영역이 만나는 새 길" / "N주 전 강한 흥미 영역에서 이어집니다"). 자세히는 [`prompts/README.md`](prompts/README.md) 진행 트래커. 별도 P2 세션: 5 persona × 실 GPT-5.5 fusion 카드 시연 + Codex R2 재감사 + A8-v2 P2-26~29 fix.
