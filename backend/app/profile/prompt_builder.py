...
def build_system_prompt(generator_version: str) -> str:
    ...
    return (
        ...
        f"    카테고리별로 분리되어 있으며 각 카테고리의 풀에서만 선택해야 한다.\n"
        f"      * cso_candidate_pool.fusion: fusion_candidates 의 bridge_cso_topic_id 선택 풀\n"
        f"      * cso_candidate_pool.deepening: deepening_seeds 의 cso_topic_id 선택 풀\n"
        f"      * cso_candidate_pool.broadening: broadening_seeds 의 cso_topic_id 선택 풀\n\n"
        f"당신의 작업 (6 필드 응답):\n"
        ...
        f"     cso_candidate_pool.fusion 안에서 선택) + bridge_reasoning (왜 두 영역이 만나는지 1-2 "
        f"     문장 한국어 설명, 20-300자). 두 영역이 너무 멀어 무의미한 bridge 라면 candidate 0개.\n"
        f"  E. deepening_seeds (0-3개): active 영역 안의 미답방 후속 영역 — cso_candidate_pool.deepening "
        f"     에서 선택. {{cso_topic_id, label}}.\n"
        f"  F. broadening_seeds (0-3개): active 영역 외 다른 cluster 중 사용자 지속 성향과 친화도가 "
        f"     높은 영역 — cso_candidate_pool.broadening 에서 선택. {{cso_topic_id, label}}.\n\n"
        f"CSO 노드 ID 매핑 강제: bridge_cso_topic_id 는 cso_candidate_pool.fusion 안, "
        f"deepening_seeds 의 cso_topic_id 는 cso_candidate_pool.deepening 안, "
        f"broadening_seeds 의 cso_topic_id 는 cso_candidate_pool.broadening 안의 UUID 만 사용. "
        f"카테고리 위반 또는 입력에 없는 새 UUID 생성 시 해당 항목이 응답에서 제거된다.\n\n"
