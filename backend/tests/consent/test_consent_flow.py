...
v13 라운드 A4 Topic-driven Pivot prep — A4 가 도입할 LLM tool-use 검색 +
/topics/{id}/documents cross-cutting + Document/CollectionJob ORM 이 ConsentGate
와 정합되어야 하므로, consent 인프라 (agreed=false 거부 / revoke 시 추천 캐시
invalidate / ConsentGate 의 onboarding 외 protected path 차단 / account-deletion
lock 지속성 / deletion 시 refresh family revoke) 회귀 가드.
"""
...
async def test_consent_revoke_invalidates_recommendation_cache(
    ...
    A4 의 LLM 검색이 추천 cache 와 정합 — revoke 후 stale cache 가 남으면 안 됨.
    """
...
async def test_consent_gate_blocks_other_protected_endpoint(
    ...
    """/topics/cso/clusters 도 ConsentGate PROTECTED_PATTERNS 적용 → 403.

    A4 가 /topics/{id}/documents 본문 채울 때 ConsentGate 가 그 endpoint 도 차단해야
    함을 보장 (PROTECTED_PATTERNS 의 /topics/.* 패턴 회귀 가드).
    """
