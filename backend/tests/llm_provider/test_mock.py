...
v13 라운드 A4 Topic-driven Pivot prep — A4 가 `LLMProvider.search_with_tools()`
신규 메서드를 추가해 LLM 호출 빈도 + 패턴이 변하므로, 기존 `complete()` 의 분산
semaphore (C-19) global/per-user cap, timeout LLMBudgetExceeded, release floor 0
invariant 회귀 가드. multi-worker 환경에서 LLM_MAX_CONCURRENT 글로벌 한도 보장.
"""
...
# ============================================================
# v13 라운드 A4 prep — Redis 분산 semaphore (C-19) 회귀 가드
# ============================================================
...
async def test_acquire_slot_caps_global_concurrent(
    ...
    A4 의 LLM 호출 (complete + search_with_tools) 이 늘어나도 multi-worker 환경에서
    global cap 이 강제됨을 보장.
    """
...
async def test_acquire_slot_caps_per_user(
    ...
    다른 user 는 영향 없음. A4 의 사용자별 trace 검색 burst 도 per-user cap 보호."""
...
async def test_acquire_slot_timeout_raises_budget_exceeded(
    ...
    A4 의 LLM 검색 호출이 timeout 시 RQ retry 흐름으로 fallback 되도록 invariant 보장.
    """
...
async def test_release_floor_zero(redis_client: aioredis.Redis) -> None:
    ...
    A4 의 search_with_tools / orchestrator 가 acquire/release 패턴 변경 시 floor
    guarantee 보존.
    """
    user_id = uuid4()
    for _ in range(3):
