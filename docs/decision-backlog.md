...
| C-40. A8 본문 + R1 self-review fix (2026-05-17) | A8 (recommendation engine) Phase 2 후반 본문. core/adjacent/discovery 3 슬롯 (5/3/2) + Cold-start LLM orchestrator + reason_short LLM batch + Document 섹션형 LLM 요약 + single-flight cache + 첫 trace 생성 hook (A7 결정 #6 plan TBD 완성). **본문**: `app/recommendation/` 11 파일 (service / engine / candidates / ranking / diversify / fallback / reasons / cold_start / summary_service / config_loader + router wire) + `app/worker/jobs/cold_start` 본문 + `app/interest/service.py` trace creation hook + `app/config/recommendation.toml` + Settings 9 신규 field. **alembic 0006**: Recommendation + RecommendationSlot + DocumentSummaryCache 3 신규 테이블 + daily UNIQUE 인덱스 + 3 CHECK. 4 commit: c82ce83 PR-1 alembic+ORM (3 model + schema.md drift fix) + 6476037 PR-2 본문 (~3514 line insertion) + ce35d8e PR-3 tests (9 파일 + conftest, 1852 line) + 15883d1 PR-4 R1 self-review fix (TopicChip dedup). **결정 매트릭스 7건 (사용자 결정 4 + 자체 결정 3)**: PR-stack 7-commit + Codex 3 라운드 (R1 self + R2/R3 별도) / Summary 캐시 = 신규 테이블 DocumentSummaryCache / 첫 trace 생성 hook = cold_start orchestrator + ingest_event_atomic click hook 협업 / PUT /onboarding/interests (FR-55) = stub 유지 (A9/A10 분담) / sentinel `cold_start_pseudo` (A2 alembic 0001 시드) + content_type='pseudo_cold_start' 활성화 / emerging quota = core 5 중 1, 부재 시 active 회수 / score 컬럼 = Recommendation DB 영속 (admin 노출), 일반 사용자 응답 schema 부재 (NFR-04). **§11 anti-pattern 5건 사전 방어** (R1 fix 최소화 목표): #1 cache-before-commit (db.commit→redis.setex 순서) / #2 daily UNIQUE race (on_conflict_do_nothing+lookup fallback) / #3 emerging quota race (candidates SQL 단일 fetch + leaf_status 컬럼) / #4 score 노출 (RecommendationCard schema 부재 + 명시 매핑) / #5 lock token race (Lua atomic CAS DEL + uuid token). **R1 self-review fix 1건** (commit 15883d1): TopicChip dedup by (topic_id, type) — 같은 doc 가 다중 confidence 로 같은 leaf 매핑 시 chip 중복 차단. R2/R3 (Codex 재감사 + 실 OpenAI 통합 시연) 은 별도 세션. | `backend/alembic/versions/0006_a8_recommendation_tables.py`, `backend/app/db/models/{recommendation, recommendation_slot, document_summary_cache, __init__}.py`, `backend/app/recommendation/* 11 파일`, `backend/app/worker/jobs/cold_start.py`, `backend/app/interest/service.py`, `backend/app/config/{__init__, recommendation.toml}`, `.env.example x2`, `docs/{data/schema, ops/env-vars}.md`, `backend/tests/recommendation/*` |
...
마지막 정리: 2026-05-17 (A8 recommendation 본문 4 commit-stack + R1 self-review fix (C-40) — c82ce83 PR-1 alembic 0006 + ORM 3 / 6476037 PR-2 본문 11 파일 (~3514 line) + cold_start worker + interest hook + Settings 9 + recommendation.toml / ce35d8e PR-3 tests 9 파일 (1852 line) / 15883d1 PR-4 R1 self-review fix (TopicChip dedup). §11 anti-pattern 5건 사전 방어 (cache-before-commit / daily UNIQUE race / emerging quota race / NFR-04 score 노출 / Lua atomic CAS DEL). R2 Codex 재감사 + R3 통합 시연 (실 OpenAI GPT-5.5) 은 별도 세션. 이전: 2026-05-17 (A6 interest-bayesian 본문 + Codex round 1 fix 8건 (C-37) + round 2 재감사 fix 4건 (C-38) — [PR #18](https://github.com/nwejnkasdf/SKKU-insight/pull/18) 머지 commit `a0a3fbf`. 합계 12건 추가 해소. docker compose 격리 통합 시연으로 alembic 0001→0004 + 22 tables + system_config_loaded + signup/consent/JWT 흐름 + `/interest/state` 200 + NFR-04 마스킹 + `/events` idempotency 200/409 + `/events/batch` 207 정합 검증. PR #19 docs drift fix (`a2930cf`) 후속). 이전: 2026-05-17 (A4 v13 코드 구현 + Codex round 2 fix 15건 (C-34) + round 3 재감사 fix 7건 (C-35) + 통합 시연 발견 fix 4건 (C-36) — [PR #16](https://github.com/nwejnkasdf/SKKU-insight/pull/16) 머지. 합계 26건 추가 해소). 이전: 2026-05-11 (P0-1·P2-6 해소 + clickbait_module 코드 phase 완료로 P1-8·P2-7 해결 — 합계 4건 해소).
