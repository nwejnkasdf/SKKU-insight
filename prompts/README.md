...
| 2 | A7 leaf-lifecycle + traversal | [`06-A7-leaf-traversal.md`](06-A7-leaf-traversal.md) | 🟢 | **7-commit PR-stack + Codex 3 라운드 audit 23건 fix** (2026-05-17): PR-1 contracts SOR (7332b34, JobType.TRACE_MERGE + ErrorCode 5 + RedisKey 3 + Settings 33) + PR-2 alembic 0005 + ORM + topic_lifecycle.toml (9f24ac3, UserCSOTraversal.merged_into_trace_id + ck_collection_job_type 갱신 P2-21 해소) + PR-3 본문 (65f415f, app/traversal 5 + app/leaf_lifecycle 6 + worker/jobs 3 신규 + scheduler 3 cron + INTEREST_PROPAGATION_ENABLED=true 토글, 3167 line) + PR-3 tests (37b686f, traversal + leaf_lifecycle 47 unit test) + Codex R1 fix 10건 (485da9b, Critical 3 + Suggested 6 + Nit 1) + Codex R2 fix 6건 (f8457b9, Critical 3 + Suggested 2 + R1 deferred S5 처리) + docs drift (cef607d) + Codex R3 fix 7건 (dbaefe6, Critical 3 + Suggested 4, R3-NEW-S1 P2 등재). **결정 매트릭스 23건** (decisions.md §12): trace operation 4 → 5 (merge 신규) / D 하이브리드 LifecycleEvaluator / Strict 검증 / input D union / split T 단축+T'=분기점+B / merge winner=max activity + tie 시 trace_id 작은 쪽 / daily 18 UTC trace merge LLM cron. **decision-backlog C-39 신규**. **C-56 leaf 활성 신호 ingest hook** ([PR #46](https://github.com/nwejnkasdf/SKKU-insight/pull/46), 2026-05-24, decisions §19): `rule_evaluator.py` 명세 "활성 신호 ingest 직후 즉시" 가 코드로 안 박혀 있던 결함 (P1-12 와 같은 패턴) fix. `interest/service.py:ingest_event_atomic` step 7.5 에 `_update_leaf_last_signal` + `_evaluate_leaf_promotion` hook 추가 — click/save/dwell_tick 시 즉시 leaf last_signal UPDATE + window 카운터 SQL + promotion (window_promotion/reactivation) only apply. 사용자 결정 3 (B/P/넓게). 회귀 가드 5. 스키마 변경 0. |
| 2 | A8 recommendation | [`07-A8-recommendation.md`](07-A8-recommendation.md) | 🟢 시연 검증 완료 | **7-commit PR-stack + Codex 3 라운드 audit fix + 실 GPT-5.5 통합 시연 검증** ([PR #23](https://github.com/nwejnkasdf/SKKU-insight/pull/23), 2026-05-17): c82ce83 PR-1 alembic 0006 + 3 ORM (Recommendation/RecommendationSlot/DocumentSummaryCache, daily UNIQUE, 3 CHECK) + 6476037 PR-2 본문 11 파일 (`app/recommendation/*` + `worker/jobs/cold_start.py` 본문 + `interest/service.py` trace creation hook + Settings 9 + `recommendation.toml` + env-vars drift, ~3514 line) + ce35d8e PR-3 tests 9 파일 + conftest (1852 line) + 15883d1 PR-4 R1 self-review fix (TopicChip dedup) + ebdd11f docs drift + 099f837 PR-5 R2 Codex 재감사 fix + ee627a2 PR-6 R3 통합 시연 fix. 결정 7건 (decisions.md §13, C-40), §11 anti-pattern 5건 사전 방어, Codex 3 라운드 결과, R3 시연 (alembic 0001→0006 + dashboard 10 cards 실 GPT-5.5 5/3/2 분배). |
| 2 | **A8-v2 discovery pivot** | (별도 prompt 없음 — decisions.md §15·§16·§17 + decision-backlog C-42·C-53·C-54 가 SOR) | 🟢 본문 + R1 audit + R1 fix + 머지 완료 + C-53 discovery 대규모 공사 + C-54 fusion fetch | **단일 commit (논리 7-PR 그룹) + Codex R1 audit + R1 fix 7건** ([PR #25](https://github.com/nwejnkasdf/SKKU-insight/pull/25) merge `63f2cdde`, 2026-05-19): discovery slot 2 본질을 "trust=high trend" → "Fusion 1 (archive × current cross-product) + Reincarnation 1 (`score_tail >= 0.6` archived trace 부활)" 로 pivot. core 5 + adjacent 3 안정성 base 유지. SRS FR-41 "잠재적으로 관심 있을 수 있는" 의도 회복. **본문**: alembic 0007 (UserProfile 10 컬럼 + ck_collection_job_type 7-value) + `app/profile/` 5 파일 (~900줄 Pydantic strict USER_PROFILE_JSON_SCHEMA) + `worker/jobs/user_profile.py` daily 19 UTC cron + scheduler 7th entry + recommendation engine pivot (`_build_discovery_pools` per-source fallback + `_build_fusion_subslot` + `_build_reincarnation_subslot` + `_resolve_seed_id` + 3 신규 query) + traversal queries 3 신규. **결정 매트릭스 21건** (사용자 11 + 자체 10). **학술 정합**: PersonaX (ACL'25) + LettinGo (KDD'25) + Serendipity 3-dim framework + 본 라운드 고유 archive×current 융합 angle. **Codex R1 audit (codex:rescue GPT-5.5)**: Critical 2 + Suggested 7 + Nit 2 = 11 issue → R1 fix 7건 + P2-26~29 backlog. **테스트**: tests/profile/ 4 파일 — 57 passed. **C-53 discovery 알고리즘 대규모 공사** ([PR #39](https://github.com/nwejnkasdf/SKKU-insight/pull/39) + [PR #40](https://github.com/nwejnkasdf/SKKU-insight/pull/40), 2026-05-24, decisions §16): Fusion bridge_cso 결정을 LLM → **trace↔trace meet-in-the-middle BFS** (LLM 의존 0 + deterministic). Reincarnation `get_top_archived_trace` → **`softmax_sample_trace`** (T=0.3). `weekly_promotion_job` 신규 (주 1회 일요일 18 UTC, save 신호 → archived→active 부활 / 새 active INSERT). `Recommendation.origin_type/origin_ref` (alembic 0010). 사용자 결정 9 + 자체 결정 5. **C-53 followup** ([PR #42](https://github.com/nwejnkasdf/SKKU-insight/pull/42), 2026-05-24): 자체 평가 빈틈 4건 fix — BFS 표준 bidirectional 패턴 + active_trace 도 softmax + weekly_promotion `traversal_lock` + `[freshness.discovery]` sub-table 폐기 + `_UNITY_FRESHNESS` 코드 상수. **C-54 fusion bridge_cso 영역 fresh Document fetch** ([PR #43](https://github.com/nwejnkasdf/SKKU-insight/pull/43), 2026-05-24, decisions §17): `apply_fusion_bridge_override` 가 BFS 결정 직후 LLM web_search + Document/DocumentTopic INSERT (D=bridge_cso 단일 매핑). 사용자 결정 6 + P1 dedup hint (직전 30일 fusion URL/title 회피) + 자체 결정 6. 신규 `backend/app/profile/fusion_fetch.py` + Settings 3 env. provider 인터페이스 미확장. 스키마 변경 0. 비용 사용자당 LLM 1회/일 추가. **WSL docker compose 통합 시연 (C-42 시점)**: alembic 0001→0007 + lifespan 부트 (`cso_graph 14707 nodes / 12 clusters`) + scheduler `JOB_REGISTRATIONS=7` + 모든 신규 모듈 surface import + signup/login/consent/clusters HTTP OK. 5 persona × 실 GPT-5.5 fusion 카드 시연 + C-53 weekly_promotion 시뮬레이션 + C-54 fetch 검증은 별도 세션. |
| 3 | A9 electron-client | [`08-A9-electron-client.md`](08-A9-electron-client.md) | 🟡 | `client/` 구축 (Vite + React + TS + Electron, `src/` + `electron/`). main 에 ~30+ 보강 커밋 머지 (슬롯 배지 / 추천 슬롯 표기 / 토픽 표시·피드백 토글 / 문서 이동·경로 / 보관함 숨김 / 관심 그래프 선 연결 / 관심 없음 피드백 범위 등). 시연 통과 검수 보류 |
| 3 | A10 admin-console | [`09-A10-admin-console.md`](09-A10-admin-console.md) | 🟡 | `admin-console/` 구축 (Next.js + `server.mjs`). main 에 ~15+ 보강 커밋 머지 (사이드바 디자인 / 브랜드·운영 항목 / 수집 상태·갱신·로딩 / 수동 수집 실행 / 최근 수집 시각 / 로그인·UI 톤 등). 시연 통과 검수 보류 |
