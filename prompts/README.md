...
| 2 | A8 recommendation | [`07-A8-recommendation.md`](07-A8-recommendation.md) | 🟢 본문 완료 (R2/R3 별도 세션) | **4-commit local PR-stack** (2026-05-17): c82ce83 PR-1 alembic 0006 + 3 ORM (Recommendation/RecommendationSlot/DocumentSummaryCache, daily UNIQUE, 3 CHECK) + 6476037 PR-2 본문 11 파일 (`app/recommendation/*` + `worker/jobs/cold_start.py` 본문 + `interest/service.py` trace creation hook + Settings 9 + `recommendation.toml` + env-vars drift, ~3514 line) + ce35d8e PR-3 tests 9 파일 + conftest (1852 line) + 15883d1 PR-4 R1 self-review fix (TopicChip dedup). **결정 매트릭스 7건** (decisions.md §13, decision-backlog C-40): PR-stack 7-commit / DocumentSummaryCache 신규 테이블 / 첫 trace 생성 hook = cold_start orchestrator + click hook (A7 #6 plan TBD 완성) / FR-55 stub 유지 / sentinel cold_start_pseudo 활성화 / emerging quota 1 + 회수 / NFR-04 score 마스킹. **§11 anti-pattern 5건 사전 방어** (R1 fix 최소화): cache-before-commit / daily UNIQUE race / emerging quota race / NFR-04 score 노출 / lock token race. **R2/R3 후속 (별도 세션)**: Codex 재감사 + 실 OpenAI GPT-5.5 cold-start 1회 통합 시연. |
