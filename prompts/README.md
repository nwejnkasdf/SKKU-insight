...
| 2 | A8 recommendation | [`07-A8-recommendation.md`](07-A8-recommendation.md) | 🟢 시연 검증 완료 | **7-commit PR-stack + Codex 3 라운드 audit fix + 실 GPT-5.5 통합 시연 검증** (2026-05-17): c82ce83 PR-1 alembic 0006 + 3 ORM (Recommendation/RecommendationSlot/DocumentSummaryCache, daily UNIQUE, 3 CHECK) + 6476037 PR-2 본문 11 파일 (`app/recommendation/*` + `worker/jobs/cold_start.py` 본문 + `interest/service.py` trace creation hook + Settings 9 + `recommendation.toml` + env-vars drift, ~3514 line) + ce35d8e PR-3 tests 9 파일 + conftest (1852 line) + 15883d1 PR-4 R1 self-review fix (TopicChip dedup) + ebdd11f docs drift + 099f837 PR-5 R2 Codex 재감사 fix (Critical #2 UTC 경계 fallback + Suggested #1 Lua atomic + P2-22/23/24 backlog) + ee627a2 PR-6 R3 통합 시연 fix (GPT-5 temperature/max_tokens 분기). **결정 매트릭스 7건** (decisions.md §13, decision-backlog C-40): PR-stack 7-commit / DocumentSummaryCache 신규 테이블 / 첫 trace 생성 hook = cold_start orchestrator + click hook (A7 #6 plan TBD 완성) / FR-55 stub 유지 / sentinel cold_start_pseudo 활성화 / emerging quota 1 + 회수 / NFR-04 score 마스킹. **§11 anti-pattern 5건 사전 방어**: cache-before-commit / daily UNIQUE race / emerging quota race / NFR-04 score 노출 / lock token race. **Codex 3 라운드 결과**: R1 1 fix + R2 Critical 2/Suggested 1/Discussion 2/Acknowledged 6 (Critical #2 + Suggested #1 즉시 fix / P2-22/24 backlog / P2-23 R3 검증 해소) + R3 시연 발견 1 fix (GPT-5 temperature). **R3 시연 검증** (docker compose + 실 OpenAI GPT-5.5 HTTP 200 61s): alembic 0001→0006 P2-23 functional index 통과 → signup → consent → onboarding 3 cluster → cold_start_job (worker) → dashboard 10 cards (실 논문/뉴스 5/3/2 분배) + Korean reason 23~31자 + NFR-04 score 미노출 + 2회차 cache hit + cold_start=true. |
