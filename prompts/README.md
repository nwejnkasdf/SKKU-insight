...
| 0b | A3 cso-topic | [`02-A3-cso-topic.md`](02-A3-cso-topic.md) | 🟢 | 3 PR stack: docs-drift (PR #1) → A2 ORM hotfix 8 모델 (PR #2) → A3 본문 (PR #3). `backend/app/topic/` 9 파일 (graph/mapping/cso_importer/cache/lifespan/cso_service/leaf_service/trace_service/router) + alembic 0002 (cso_topic_parent + dynamic_leaf_topic + dynamic_leaf_topic_cso_topic) + `backend/scripts/import_cso.py` + `backend/app/config/broad_interests.toml` 12 entry + 7 endpoint 본문 (clusters/detail/adjacent/descendants/leaves/leaves-detail/traces/traces-detail, documents 만 NotImplementedError 유지) + tests/topic 45 unit tests. `ruff` · `mypy --strict 99 files` · 6 cross-check 통과. |
