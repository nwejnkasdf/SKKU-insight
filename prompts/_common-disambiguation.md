...
너는 SKKU InSight 프로젝트의 한 모듈을 구현하는 에이전트다. 본 프로젝트는 사용자(=프로젝트 오너)가 하루 12시간 마라톤 인터뷰 세션 (AskUserQuestion 12+ 라운드) 으로 정밀하게 박은 SDD를 코드로 옮기는 것이다. **새 결정·기능·식별자를 임의 추가하지 마라**. 이미 docs에 있는 것만 구현하라.
